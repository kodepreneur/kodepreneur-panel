...
<template>
    <AppLayout title="Deployments">
        <div class="space-y-6 max-w-7xl mx-auto">
            <!-- Header -->
            <div>
                <h2 class="text-base font-bold text-slate-900 dark:text-white tracking-tight">Deployment Pipeline</h2>
                <p class="text-xs text-slate-500 dark:text-surface-400 mt-0.5">Track real-time Git build logs, execution durations, and release histories</p>
            </div>
            ...
            <div class="rounded-2xl bg-white dark:bg-surface-900/60 border border-slate-200/80 dark:border-surface-800/80 shadow-sm dark:shadow-xl overflow-hidden">
                <div v-if="deployments.data.length === 0" class="text-center py-12 px-4">
                    <GitBranch class="w-10 h-10 text-slate-300 dark:text-surface-600 mx-auto mb-3" />
                    <h3 class="text-sm font-medium text-slate-700 dark:text-surface-200">No deployment runs recorded yet</h3>
                ...
                <div v-else class="overflow-x-auto">
                    <table class="w-full text-left text-xs">
                        <thead class="bg-slate-50 dark:bg-surface-950/40 text-slate-500 dark:text-surface-400 uppercase text-[10px] tracking-wider border-b border-slate-200/80 dark:border-surface-800">
                            <tr>
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-surface-800/60">
                            ...
                            >
                                ...
                                <td class="py-3.5 px-4 font-mono text-[11px] text-slate-600 dark:text-surface-300">
                                    <span class="px-2 py-0.5 rounded-md bg-slate-100 dark:bg-surface-800 border border-slate-200 dark:border-surface-700">
                                        {{ deploy.branch }}
                                    </span>
                                </td>
                                <td class="py-3.5 px-4 text-slate-500 dark:text-surface-400">
                                ...
                                <td class="py-3.5 px-4">
                                    ...
                                    <span
                                        v-else
                                        class="inline-flex items-center gap-1 text-[11px] text-rose-600 dark:text-rose-400 font-medium"
                                    >
                                ...
                                <td class="py-3.5 px-4 text-right">
                                    <Link
                                        ...
                                        class="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-surface-800 dark:hover:bg-surface-700 text-slate-700 dark:text-surface-200 text-[11px] font-mono transition"
                                    >
                                        <span>Terminal</span>
