...
<template>
    <AppLayout :title="`Deployment #${deployment.id}`">
        <div class="max-w-6xl mx-auto space-y-6">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-surface-400 mt-0.5">
                            Target:
                            <Link :href="`/websites/${deployment.website.id}`" class="text-brand-400 hover:underline font-mono">
                                {{ deployment.website.domain }}
                            </Link>
                            • Branch: <span class="font-mono text-surface-300">{{ deployment.branch }}</span>
                            <span v-if="deployment.commit_hash"> • Commit: <code class="text-emerald-400 font-mono">{{ deployment.commit_hash }}</code></span>
                        </p>
                    </div>
                </div>
            ...
            <div class="rounded-2xl bg-surface-950 border border-surface-800 shadow-2xl overflow-hidden">
                <!-- Terminal Titlebar -->
                <div class="h-10 bg-surface-900/90 border-b border-surface-800 px-4 flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        <div class="w-3 h-3 rounded-full bg-rose-500/80"></div>
                        <div class="w-3 h-3 rounded-full bg-amber-500/80"></div>
                        <div class="w-3 h-3 rounded-full bg-emerald-500/80"></div>
                        <span class="text-xs text-surface-400 font-mono ml-2">runner@kodepreneur (~/{{ deployment.website.domain }})</span>
                    </div>
                    <span class="text-[11px] text-surface-400 font-mono flex items-center gap-1">
