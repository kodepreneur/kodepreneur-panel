    ...
    <AppLayout title="Firewall">
        <div class="max-w-7xl mx-auto space-y-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h2 class="text-base font-bold text-slate-900 dark:text-white tracking-tight">UFW Firewall Manager</h2>
            ...
            <div class="rounded-2xl bg-white dark:bg-surface-900/60 border border-slate-200/80 dark:border-surface-800/80 shadow-sm dark:shadow-xl overflow-hidden">
                <div v-if="rules.data.length === 0" class="text-center py-14 px-4">
                    <Shield class="w-10 h-10 text-slate-300 dark:text-surface-600 mx-auto mb-3" />
                    <h3 class="text-sm font-medium text-slate-700 dark:text-surface-200">No custom firewall rules</h3>
                ...
                <div v-else class="overflow-x-auto">
                    <table class="w-full text-left text-xs">
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-surface-800/60">
                            ...
                            >
                                <td class="py-3.5 px-4 font-semibold text-slate-900 dark:text-white font-mono text-xs">
                                ...
                                <td class="py-3.5 px-4 font-mono text-[11px] uppercase font-semibold text-slate-700 dark:text-surface-300">
                                    {{ r.protocol }}
                                </td>
                                ...
                                <td class="py-3.5 px-4 text-[11px] text-slate-700 dark:text-surface-300">
                                    {{ r.description || '—' }}
                                </td>
            ...
            >
                <div class="w-full max-w-md rounded-2xl bg-white dark:bg-surface-900 border border-slate-200/90 dark:border-surface-800 p-6 shadow-2xl space-y-4">
                    <h3 class="text-sm font-semibold text-slate-900 dark:text-white">Add Firewall Rule</h3>

                    <form @submit.prevent="submitAddRule" class="space-y-4">
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label class="block text-xs font-medium text-slate-700 dark:text-surface-300 mb-1">Port</label>
                                <input
                            ...
                            <div>
                                <label class="block text-xs font-medium text-slate-700 dark:text-surface-300 mb-1">Protocol</label>
                                <select
                        ...
                        <div>
                            <label class="block text-xs font-medium text-slate-700 dark:text-surface-300 mb-1">Action</label>
                            <select
                        ...
                        <div>
                            <label class="block text-xs font-medium text-slate-700 dark:text-surface-300 mb-1">Source IP (Optional)</label>
                            <input
                        ...
                        <div>
                            <label class="block text-xs font-medium text-slate-700 dark:text-surface-300 mb-1">Description</label>
                            <input
