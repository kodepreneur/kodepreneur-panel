...
<template>
    <AppLayout title="System Services">
        <div class="space-y-6 max-w-7xl mx-auto">
            <!-- Header -->
            <div>
                <h2 class="text-base font-semibold text-white tracking-tight">Systemd Services</h2>
                <p class="text-xs text-surface-400 mt-0.5">Control web servers, database daemons, and PHP-FPM pools</p>
            </div>
                ...
                >
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2.5">
                            <div class="w-8 h-8 rounded-lg bg-surface-800 border border-surface-700 text-brand-400 flex items-center justify-center font-mono text-xs">
                                <Cpu class="w-4 h-4" />
                            </div>
                            <div>
                                <h3 class="text-xs font-semibold text-white font-mono">{{ svc.name }}</h3>
                                <span class="text-[10px] text-surface-500 font-mono">systemd unit</span>
                            </div>
                        </div>

                        <span class="flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-medium bg-emerald-500/10 border border-emerald-500/30 text-emerald-400">
                            <span class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
                            Active
                        </span>
                    </div>

                    <div class="flex items-center gap-2 pt-2 border-t border-surface-800/60">
                        <button
