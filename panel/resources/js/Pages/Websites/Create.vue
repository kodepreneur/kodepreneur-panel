    ...
    <AppLayout title="Create Website">
        <div class="max-w-3xl mx-auto space-y-6">
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-base font-semibold text-white tracking-tight">Provision New Virtual Host</h2>
            ...
            <div class="rounded-2xl bg-surface-900/60 border border-surface-800/80 p-6 sm:p-8 shadow-xl">
                <form @submit.prevent="submit" class="space-y-6">
                    <!-- Domain Name -->
                    <div>
                        <label class="block text-xs font-semibold text-surface-200 mb-1.5 flex items-center justify-between">
                            <span>Primary Domain Name <span class="text-rose-400">*</span></span>
                            <span class="text-[11px] text-surface-500 font-normal">e.g. example.com or app.mydomain.io</span>
                        </label>
                        <div class="relative">
                            <Globe class="w-4 h-4 text-surface-500 absolute left-3.5 top-1/2 -translate-y-1/2" />
                            <input
                                v-model="form.domain"
                                @input="updateDocRoot"
                                @blur="updateDocRoot"
                        ...
                        <p v-if="form.errors.domain" class="text-[11px] text-rose-400 mt-1.5">{{ form.errors.domain }}</p>
                    </div>

                    <!-- PHP Runtime Selector (Card Grid) -->
                    <div>
                        <label class="block text-xs font-semibold text-surface-200 mb-2">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                            ...
                            >
                                <div class="flex items-center justify-between">
                                    ...
                                    <CheckCircle2 v-if="form.php_version === '8.3'" class="w-4 h-4 text-brand-400" />
                                </div>
                                <p class="text-[10px] text-surface-400 mt-2">Recommended default. Long-term support.</p>
                            </label>
                            ...
                            >
                                <div class="flex items-center justify-between">
                                    ...
                                    <CheckCircle2 v-if="form.php_version === '8.4'" class="w-4 h-4 text-brand-400" />
                                </div>
                                <p class="text-[10px] text-surface-400 mt-2">Latest release with property hooks & JIT.</p>
                            </label>
                            ...
                            >
                                <div class="flex items-center justify-between">
                                    ...
                                    <CheckCircle2 v-if="form.php_version === 'none'" class="w-4 h-4 text-brand-400" />
                                </div>
                                <p class="text-[10px] text-surface-400 mt-2">Static HTML, SPA, or Node.js proxy.</p>
                            </label>
                        </div>
                        <p v-if="form.errors.php_version" class="text-[11px] text-rose-400 mt-1.5">{{ form.errors.php_version }}</p>
                    </div>

                    <!-- Document Root -->
                    <div>
                        <label class="block text-xs font-semibold text-surface-200 mb-1.5">
                            Document Root (Web Directory)
                        </label>
                        <div class="relative">
                            <Folder class="w-4 h-4 text-surface-500 absolute left-3.5 top-1/2 -translate-y-1/2" />
                            <input
                        ...
                        <p class="text-[11px] text-surface-500 mt-1.5">
                            Point to Laravel's <code class="text-brand-400 font-mono">/public</code> directory or custom application webroot.
                        </p>
                        <p v-if="form.errors.document_root" class="text-[11px] text-rose-400 mt-1.5">{{ form.errors.document_root }}</p>
                    </div>

                    <!-- Automated SSL Option -->
                    <div class="p-4 rounded-xl bg-surface-950/40 border border-surface-800/80 space-y-3">
                        ...
                        <div v-if="form.auto_ssl" class="pt-2">
                            <label class="block text-[11px] font-medium text-surface-400 mb-1">
