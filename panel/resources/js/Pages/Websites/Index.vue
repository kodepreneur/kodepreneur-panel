    ...
    <AppLayout title="Websites">
        <div class="space-y-6 max-w-7xl mx-auto">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h2 class="text-base font-semibold text-white tracking-tight">Managed Websites</h2>
                ...
                <Link
                    ...
                    class="px-4 py-2 rounded-xl bg-brand-600 hover:bg-brand-500 text-white text-xs font-semibold shadow-lg shadow-brand-600/20 flex items-center gap-1.5 transition self-start sm:self-auto"
                >
                    <Plus class="w-4 h-4" />
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div class="relative">
                    ...
                    <input
                        ...
                        class="w-full pl-9 pr-4 py-2 rounded-xl bg-surface-900/60 border border-surface-800 text-xs text-white placeholder-surface-500 focus:outline-none focus:ring-2 focus:ring-brand-500/40 focus:border-brand-500 transition"
                    />
                </div>
                ...
                <div class="flex items-center gap-2">
                    <select
                        v-model="filterPhp"
                        class="w-full px-3 py-2 rounded-xl bg-surface-900/60 border border-surface-800 text-xs text-surface-300 focus:outline-none focus:ring-2 focus:ring-brand-500/40 transition"
                    >
            ...
            <div class="rounded-2xl bg-surface-900/60 border border-surface-800/80 shadow-lg overflow-hidden">
                <div v-if="filteredWebsites.length === 0" class="text-center py-14 px-4">
                    <Globe class="w-10 h-10 text-surface-600 mx-auto mb-3" />
                    <h3 class="text-sm font-medium text-surface-200">No websites found</h3>
                    ...
                    <Link
                        v-if="!searchQuery"
                        href="/websites/create"
                        class="mt-4 inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-brand-600 text-white text-xs font-semibold hover:bg-brand-500 transition shadow-lg shadow-brand-600/20"
                    >
                    ...
                    <table class="w-full text-left text-xs">
                        ...
                        <tbody class="divide-y divide-surface-800/60">
                            <tr
                                ...
                                :key="site.id"
                                class="hover:bg-surface-800/30 transition group"
                            >
                                <td class="py-3.5 px-4">
                                    <div class="flex items-center gap-2.5">
                                        <div class="p-2 rounded-xl bg-brand-500/10 border border-brand-500/20 text-brand-400 shrink-0">
                                            <Globe class="w-4 h-4" />
                                        </div>
                                        <div>
                                            <Link
                                                :href="`/websites/${site.id}`"
                                                class="font-semibold text-surface-100 hover:text-brand-400 transition"
                                            >
                                                {{ site.domain }}
                                            </Link>
                                            <span class="block text-[10px] text-surface-500 font-mono">
                                ...
                                <td class="py-3.5 px-4">
                                    <span
                                        ...
                                        class="px-2 py-0.5 rounded-md bg-surface-800 border border-surface-700 text-surface-200 font-mono text-[11px]"
                                    >
                                        PHP {{ site.php_version }}
                                    </span>
                                    <span
                                        v-else
                                        class="px-2 py-0.5 rounded-md bg-surface-800/50 border border-surface-700/50 text-surface-400 font-mono text-[11px]"
                                    >
                                        Static
                                    </span>
                                </td>
                                <td class="py-3.5 px-4 font-mono text-[11px] text-surface-400 truncate max-w-xs">
                                    {{ site.document_root }}
                                </td>
                                <td class="py-3.5 px-4">
                                    <span
                                ...
                                <td class="py-3.5 px-4">
                                    <span class="px-2.5 py-0.5 rounded-full text-[10px] font-medium bg-emerald-500/10 border border-emerald-500/30 text-emerald-400">
                                        {{ site.status }}
                                    </span>
                                </td>
                                <td class="py-3.5 px-4 text-right">
                                    <div class="flex items-center justify-end gap-1.5">
                                        <Link
                                            :href="`/websites/${site.id}`"
                                            class="px-2.5 py-1.5 rounded-lg bg-surface-800 hover:bg-surface-700 text-surface-200 text-xs font-medium flex items-center gap-1 transition"
            ...
            >
                <div class="w-full max-w-md rounded-2xl bg-surface-900 border border-surface-800 p-6 shadow-2xl space-y-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="text-sm font-semibold text-white">Delete Website</h3>
                            <p class="text-xs text-surface-400 mt-0.5">
                                Are you sure you want to delete <strong class="text-white">{{ siteToDelete?.domain }}</strong>?
                            </p>
                        </div>
                    </div>

                    <p class="text-xs text-surface-400 bg-surface-950/60 p-3 rounded-xl border border-surface-800/80 leading-relaxed">
                    ...
                    <div class="flex items-center justify-end gap-2.5 pt-2">
                        ...
                        <button
                            @click="executeDelete"
                            class="px-4 py-2 rounded-xl bg-rose-600 hover:bg-rose-500 text-white text-xs font-semibold shadow-lg shadow-rose-600/20 transition"
                        >
                            Delete Website
                        </button>
                    </div>
